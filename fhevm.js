...
// privatekey
const privateKey = JSON.parse(fs.readFileSync("./privatekey.json"));
...
const main = async () => {
  ...
  // requestはsignerからのみ送れるが、signerはeip-712を当該versionで利用できない
	const signature = await web3.eth.signTypedData(token, userAddress)
//   コントラクトの呼び出しと結果の復号;
